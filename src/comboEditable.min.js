...
*/
(function(b){var h=2;if(b.browser.msie)var e="22px",f="1px";else e="20px",f="2px";var g=function(a,c){this.el=a;this.name=a.attr("name")?a.attr("name"):"";this.selectedText=this.selectedValue=null;this.elements=this.getOptionValues(a.find("option"));this.$ret=b("<div/>").css({display:a.css("display"),width:a.width()});this.opts=this.processOptions(c,a)};b.extend(g.prototype,{processOptions:function(a){var b={};if(a&&a.mode==="text")b.valueAsText=!0;return b},getOptionValues:function(){var a=this._mapOptionsToElements();
if(!this.selectedText)this.selectedValue=this.el.find("option").first().val(),this.selectedText=this.el.find("option").first().text();var c="";b.each(a,function(a,b){c+=b});return c},_mapOptionsToElements:function(){return b.map(this.el.find("option"),b.proxy(function(a){if(a.selected)this.selectedValue=a.value,this.selectedText=a.innerHTML;return'<div class="ui-state-default" data-value="'+a.value+'" '+ +(a.selected?NaN:0)+">"+a.innerHTML+"</div>"},this))},makeTextBox:function(){var a=this.$ret.append("<div/>").children().first();
a.css({position:"relative"});a.append('<input type="text" value="'+this.selectedText+'" />');this.$text=a.find(":text").css({display:"inline",width:this.el.width()-20});this.$textWrap=a},makeIcon:function(){this.$textWrap.append("<div/>");this.$button=this.$textWrap.find("div").addClass("ui-icon ui-icon-triangle-1-s ui-state-default ui-corner-right").css({height:e,width:e});this.$button.wrap("<div/>").parent().css({position:"relative",top:f,right:"1px"}).wrap("<div/>").parent().css({display:"inline",
position:"absolute"})},addHiddenField:function(){this.$ret.append('<input type="hidden" value="'+this.selectedValue+'" name="'+this.name+'" />');this.$value=this.$ret.find('input[type="hidden"]')},stylizeOptions:function(){this.$ret.append('<div class="ui-combo-editable-options"/>');var a=this.$ret.find("div.ui-combo-editable-options");a.append(this.elements);a.css({"z-index":1,position:"absolute"});a.find("div").css({display:"block",cursor:"default"}).hover(function(){b(this).addClass("ui-state-hover")},
function(){b(this).removeClass("ui-state-hover")}).css({width:this.el.width()});this.$options=a},stylizeIcon:function(){var a=this.$options;a.hide();this.$button.click(function(){a.is(":visible")?a.hide():a.show()}).hover(function(){b(this).addClass("ui-state-hover")},function(){b(this).removeClass("ui-state-hover")})},wireupHideActions:function(){var a=this.$options;this.$ret.blur(function(){a.hide()}).attr("tabindex","999").css({outline:"none"});this.$ret.keyup(function(b){b.which===27&&a.hide()});
...
h+"px"))}var b=this.$button;a("x");a("y")}});b.fn.comboEditable=function(a){a=new g(this,a);a.makeTextBox();a.makeIcon();a.addHiddenField();a.stylizeOptions();a.stylizeIcon();a.wireupHideActions();a.setFormValues();var b=this.attr("id");this.replaceWith(a.$ret);a.$text.attr("id",b);a.stretchIcon();return a.$ret}})(jQuery);
